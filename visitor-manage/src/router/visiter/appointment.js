...
class Appointment extends Component {
  ...
  onFinish = (values) => {
    ...
    values.create_time = moment(values.create_time).format('YYYY-MM-DD HH:mm')
    let newObj = {}
    Object.assign(newObj,values,{'companyName':this.state.companyName,'type':0})
    $.ajax({
      url:'http://localhost:3200/appointment',
      type: 'post',
      data: newObj,
      success:(data)=>{
        if(data.status == 0){
    ...
    return (
        <div className="appointment">
            ...
            >
                ...
                >
                    <Input placeholder="您的邮箱" autoComplete="off"/>
                </Form.Item>
