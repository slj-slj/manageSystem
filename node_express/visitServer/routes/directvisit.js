...
var directvisitRouter = express.Router();
var connection = require('./mysql')
/* GET home page. */
directvisitRouter.post('/', function(req, res, next) {
    let params = req.body
                ...
                connection.query(queryPhone,function(err4,result4){
                    if(err4) throw err4
                        ...
                        if(result4.length !==0){
                            ...
                            let query2 = 'select * from visiter'
                            connection.query(query2,function(err2,result2){
                                if(err2) throw err2
                                else{
                                    if(result2.length == 0){
                                        let queryInsert = `insert into visiter (visiter_id,visiter_name,visiter_phone,visiter_email,create_time,visited_phone,visited_name,companyName,companyId) values ('1000000001','${visiter_name}','${visiter_phone}','${visiter_email}','${create_time}','${visited_phone}','${visited_name}','${companyName}','${companyId}')`
                                        ...
                                        let queryInsert = `insert into visiter (visiter_name,visiter_phone,visiter_email,create_time,visited_phone,visited_name,companyName,companyId) values ('${visiter_name}','${visiter_phone}','${visiter_email}','${create_time}','${visited_phone}','${visited_name}','${companyName}','${companyId}')`
                                        connection.query(queryInsert,function(err3,result3){
                                            if(err3) throw err3
                                            else{
                                                res.send({
...
module.exports = directvisitRouter;
